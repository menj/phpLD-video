		...
		if (isset($this->asClassRule[$sField])){
			...
			foreach ($this->asClassRule[$sField] as $sRule => $asRuleParam){
				if ($sRule == 'remote') {
					...
					if (false === $this->oMethodCollection->{$sRule}($asRuleParam['data']['action'], $asRuleParam['data']['table'], $asRuleParam['data']['field'], $mValue)){
						$this->asErrorMessage[$sField] = $this->getMessage($sField, $sRule, 1);
...
class ValidatorMethodCollection extends ArrayObject {
  ...
  (
	...
	public function remote($func, $table, $field, $sValue){
		$f = "validator_".$func;
		if (is_callable($f)) {
			$ret  = $f($table, $field, $sValue);
			if ($ret) {
				return true;
			}
		} 
	...
	public function required($sValue, $mParam = null){
		...
		return false === $this->_empty($sTrimmedValue);
	}
